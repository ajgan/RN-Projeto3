...
sm = sompy.SOMFactory().build(data.values, normalization = 'var', initialization='pca', component_names=names)
...
view2D  = sompy.visualization.mapview.View2D(10,10,"rand data",text_size=12)
...
labels = sm.cluster(K_opt)
import sklearn.cluster as clust
km = clust.KMeans(n_clusters=K_opt).fit(
            sm._normalizer.denormalize_by(sm.data_raw,
                                            sm.codebook.matrix))
hits  = sompy.visualization.hitmap.HitMapView(20,20,"Clustering",text_size=12)
a=hits.show(sm)

# U-matrix plot
# print("umatrix")
#
# umat  = sompy.visualization.umatrix.UMatrixView(width=10,height=10,title='U-matrix')
# umat.show(sm)
